...
// github.com/giantswarm/selfupdate-cosign. What follows checks that mcp-debug
...
func (r fakeRelease) GetURL() string {
	return "https://github.com/giantswarm/mcp-debug/releases/tag/" + r.tag
}
...
func binaryAsset() string {
	name := "mcp-debug-" + runtime.GOOS + "-" + runtime.GOARCH
...
func selfUpdateFixture(t *testing.T, src *fakeSource) (string, []byte) {
	t.Helper()
	installed := []byte("the mcp-debug that is installed right now")
	exe := filepath.Join(t.TempDir(), "mcp-debug")
...
func TestRunSelfUpdateRefusesAReleaseWithoutASignatureBundle(t *testing.T) {
	src := &fakeSource{
		...
		assets:  map[int64][]byte{1: []byte("a newer mcp-debug, unsigned")},
	}
...
func TestRunSelfUpdateInstallsAVerifiedReleaseInPlace(t *testing.T) {
	...
	src := &fakeSource{
		...
		assets: map[int64][]byte{1: []byte("a newer mcp-debug"), 2: []byte("its bundle")},
	}
	...
	got, err := os.ReadFile(exe) //nolint:gosec // the test's own temp file
	if err != nil || string(got) != "a newer mcp-debug" {
