...
func TestExtractScopesFromChallenge(t *testing.T) {
	tests := []struct {
		name      string
		challenge *WWWAuthenticateChallenge
		want      []string
		...
		{
			name:      "nil challenge returns nil",
			challenge: nil,
			want:      nil,
		},
		{
			name: "challenge with scopes",
			challenge: &WWWAuthenticateChallenge{
		...
		{
			name: "challenge with empty scopes",
			challenge: &WWWAuthenticateChallenge{
				Scopes: []string{},
			},
			want: []string{},
		...
		{
			name: "challenge with nil scopes",
			challenge: &WWWAuthenticateChallenge{
				Scopes: nil,
			},
			want: nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := extractScopesFromChallenge(tt.challenge)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("extractScopesFromChallenge() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestExtractScopesFromMetadata(t *testing.T) {
	tests := []struct {
		name     string
		metadata *ProtectedResourceMetadata
		want     []string
		...
		{
			name: "metadata with scopes",
			metadata: &ProtectedResourceMetadata{
				ScopesSupported: []string{"files:read", "user:profile"},
			},
			want: []string{"files:read", "user:profile"},
		},
		{
			name: "metadata with empty scopes",
			metadata: &ProtectedResourceMetadata{
				ScopesSupported: []string{},
			},
			want: []string{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := extractScopesFromMetadata(tt.metadata)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("extractScopesFromMetadata() = %v, want %v", got, tt.want)
			}
		})
	}
}
